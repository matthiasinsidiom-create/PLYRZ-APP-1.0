  ...
  if (success) {
    return (
      <div className="min-h-screen bg-transparent flex flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="text-3xl font-black italic uppercase tracking-tight text-white mb-4">Card Claimed!</h1>
        <p className="text-zinc-500 max-w-xs">Your player card has been successfully linked to your account. Redirecting to dashboard...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-transparent text-white font-sans p-6">
      <div className="max-w-3xl mx-auto space-y-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black italic uppercase tracking-tight">Claim Your Card</h1>
            <p className="text-zinc-500 text-sm">Find your existing player card in our database.</p>
          </div>
        </div>
          ...
          <div className="grid gap-4">
            {filteredPlayers.length > 0 ? (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-zinc-500 text-xs uppercase tracking-widest font-bold">
                        {player.position} • {team?.name} • {club?.name}
                      </p>
                    </div>
                    <button 
                    ...
                    >
                      {claimingId === player.id ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Claim'}
                    </button>
                  </motion.div>
                );
              })
              ...
              <div className="text-center py-12 space-y-4">
                ...
                <p className="text-zinc-500">No unclaimed players found matching your search.</p>
              </div>
            )}
          </div>
        )}
