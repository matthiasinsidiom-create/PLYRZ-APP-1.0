...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    try {
      ...
      if (!data) {
        ...
        const { data: newProfile, error: createError } = await supabase
          ...
          .insert({
            id: userId,
            display_name: session.user.user_metadata?.display_name || session.user.email?.split('@')[0] || 'New Player',
            user_type: 'fan',
            is_admin: isAdmin
          })
          .select()
  ...
  return (
    <AuthContext.Provider value={{
      ...
      profile,
      loading,
      isAdmin: (profile?.is_admin ?? false) || (session?.user?.email === "matthias.insidiom@gmail.com"),
      profileError,
      signOut,
      refreshProfile
    }}>
