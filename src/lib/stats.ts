...
export function resolveLatestStats(player: Partial<Player>): PlayerStats {
  ...
  // Sort by updated_at descending, with fallback to id if dates are equal
  ...
  const sorted = [...statsArray].sort((a, b) => {
    ...
    // If dates are equal or missing, try to use ID as a tie-breaker
    if (a.id && b.id) {
  ...
  console.log(`DEBUG: [STATS] Selected latest row for ${player.full_name || player.id}: ID=${latest.id}, Updated=${latest.updated_at}, OVR=${latest.overall}`);
