-- Migration: Allow voting when at least one team has a lineup (instead of requiring both teams)
...
BEGIN
  ...
  IF NEW.status = 'finished' AND (OLD.status IS DISTINCT FROM 'finished') THEN
    ...
    -- If NEITHER team has lineup players, skip voting and mark as directly processed!
    IF v_home_lineup_count = 0 AND v_away_lineup_count = 0 THEN
    ...
    -- If we have lineups for at least one team, set voting window:
...
BEGIN
    ...
    -- If BOTH teams have 0 players in lineup: finish match WITHOUT voting
    IF v_home_lineup_count = 0 AND v_away_lineup_count = 0 THEN
        ...
        RETURN json_build_object(
            ...
            'status', 'finished',
            'success', true,
            'message', 'Match finished directly without voting (no lineups found)'
        );
    END IF;
